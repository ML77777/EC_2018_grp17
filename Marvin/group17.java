...
public class group17 implements ContestSubmission
{
	...
	public void run()
	{
		...
		int generation_cycle = 0, elim_island_interval = 25,lowest_fitnessIsland_index = 0;
		...
		double[] fitnessOfIslands = new double[numberOfIslands];
		double lowest_fitnessIsland = -1.0;
      ...
      generation_cycle++;
			if (generation_cycle == elim_island_interval){
          generation_cycle = 0;
          for (int i = 0; i < numberOfIslands;i++) {
              fitnessOfIslands[i] = islandArray[i].getSumFitnessPopulation(); //Get sum of population for each island and put it into array
              // Keep track which island had worst fitness
					    if (i == 0 || lowest_fitnessIsland > fitnessOfIslands[i]) {
								lowest_fitnessIsland = fitnessOfIslands[i];
								lowest_fitnessIsland_index = i;
							}
					}

					System.out.println(Arrays.toString(fitnessOfIslands));
					System.out.println("Lowest index " + lowest_fitnessIsland_index + " en value " + fitnessOfIslands[lowest_fitnessIsland_index]);
					System.out.println(Arrays.toString(islandArray));
					//Then if numberOfIslands > 1 then elimnate the worst one
          if (numberOfIslands > 1) {
						numberOfIslands--;
					Island[] tmpislandArray = new Island[numberOfIslands];
					int tmp_index = 0;
					for (int i = 0; i< numberOfIslands + 1;i++){
						if (i != lowest_fitnessIsland_index){
                tmpislandArray[tmp_index] = islandArray[i];
								tmp_index++;
						}
					}
					islandArray = tmpislandArray;
					System.out.println(Arrays.toString(islandArray));
				  }
			}
			System.out.println("Generation cycle: " + generation_cycle);
		}
